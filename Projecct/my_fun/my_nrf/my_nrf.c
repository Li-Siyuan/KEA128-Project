...
#define NRF_CSN_HIGH   gpio_set(D0,1)  //片选信号，推挽输出  
#define NRF_CSN_LOW    gpio_set(D0,0) 
#define NRF_IRQ        gpio_get(E3)//PDin(3)  //IRQ主机数据输入,上拉输入
#define NRF_SCK_HIGH   gpio_set(H7,1)  //发送接收模式选择推挽输出
#define NRF_SCK_LOW   gpio_set(H7,0)
#define NRF_CE_HIGH    gpio_set(D1,1)  //PC5 推完输出
#define NRF_CE_LOW     gpio_set(D1,0)
#define NRF_MOSI_HIGH  gpio_set(H6,1)//PBout(15)  //PC6 MOSI 主机推挽输出 （根据速率初始化PC_CR2寄存器）
...
uchar NRF24L01_TxPacket(uchar *txbuf)
{
	...
	NRF_CE_LOW;												//CE拉低，使能24L01配置
  	NRF24L01_Write_Buf(WR_TX_PLOAD,txbuf,TX_PLOAD_WIDTH);	//写数据到TX BUF  32个字节
 	NRF_CE_HIGH;												//CE置高，使能发送	   
