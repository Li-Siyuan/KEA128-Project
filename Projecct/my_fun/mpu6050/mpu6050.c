...
#define	_CONFIG			0x1A	//低通滤波器，典型值：0x06(5Hz)
...
void MPU6050_MoniI2c_Config(void)
{
	...
	MoniI2c_WriteByteToSlave(SlaveAddress,ACCEL_CONFIG, 0x10);			//设置加速度传感器量程为：±2g
	MoniI2c_WriteByteToSlave(SlaveAddress,SMPLRT_DIV, 0x04);			//设置采样率的分频值为19，从而采样率为50Hz
	MoniI2c_WriteByteToSlave(SlaveAddress,_CONFIG, 0x04);				//设置数字低通滤波值，滤波带宽为20Hz
